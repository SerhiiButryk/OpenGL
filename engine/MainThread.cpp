...
void MainThread::loop(InternalApplication* app) {
    ...
    // Create client UI
    mainApp->getClientUI()->onCreateUI();
    ...
    while (!glfwWindowShouldClose(mainApp->getWindow()))
    {
        // Render client UI
        mainApp->getClientUI()->onRenderUI();
        ...
        */
        ...
        glfwSwapBuffers(mainApp->getWindow());
    ...
    // Destroy client UI
    mainApp->getClientUI()->onDestroyUI();
